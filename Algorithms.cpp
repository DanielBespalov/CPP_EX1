...
void bfs(Graph& g, unsigned long start, std::vector<bool>& visited, std::vector<int>& dist, std::vector<int>& prev) {
    std::queue<int> q;
    q.push(start);
    visited[start] = true;
    dist[start] = 0;
    ...
    while (!q.empty()) {
        unsigned int node = (unsigned int)q.front();
        q.pop();
...
bool bellmanFord(Graph& g, std::vector<int>& dist) {
...
std::string Algorithms::shortestPath(Graph g, unsigned long start, unsigned long end) {
    unsigned int v = (unsigned int)g.getVerts();
    ...
    std::vector<int> path;
    for (int at = end; at != -1; at = prev[(unsigned int)at]) {
        path.push_back(at);
    }
...
bool Algorithms::dfsCycleCheck(Graph g, unsigned long v, std::vector<bool>& visited, int parent) {
    visited[v] = true;
...
bool Algorithms::bfsCheckBipartite(Graph g, unsigned long start, std::vector<int>& color, std::vector<int>& setA, std::vector<int>& setB) {
    std::queue<int> q;
    q.push(start);
...
std::string Algorithms::isBipartite(Graph g) {
    int numVertices = (int)g.getVerts();
